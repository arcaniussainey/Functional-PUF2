...
## Immutable OperationSpec DSL

The refactor now includes a lower-overhead composition layer in `Pufs.operations`.
The older `Pufs.pipeline` API still works, but new code should prefer immutable
operation specs when an operation sequence may run in a hot path or across many
experiments.

The key rule is:

> `OperationSpec` stores static operation metadata; runtime arrays live in
> `OperationState`.

This keeps operation sequences hashable and cacheable.  It also avoids putting
weights, challenges, or responses into the JAX compilation cache key.
...
from Pufs.FunctionalPuf import Xor, generate_challenges
...
puf = Xor(rng, (3, 128))
challenge = generate_challenges(rng, (10_000, 128))

pipeline = op_pipeline(op_evaluate_xor_response())
state = pipeline.run(
    OperationState(rng=rng, weight=puf.weight, challenge=challenge),
    jit=True,
)
individual, xor_response = state.response
```

To add a new operation, register a handler instead of editing the executor:

```python
from dataclasses import replace
import jax.numpy as jnp

from Pufs.operations import OperationSpec, OperationState, register_operation


def scale_weight(state: OperationState, spec: OperationSpec) -> OperationState:
    if state.weight is None:
        raise ValueError("scale_weight: weight must be set first")
    factor = jnp.float32(spec.get("factor"))
    return replace(state, weight=state.weight * factor)


register_operation("scale_weight", scale_weight)
scale_spec = OperationSpec.create("scale_weight", factor=0.95)
```

For compatibility, `Pufs.pipeline.compose(...)` accepts either old step callables
or `OperationSpec` values.  A pipeline can run with `compiled=True` only when all
steps are spec-backed.  Binding steps such as `use_weights(weight)` and
`use_challenges(challenge)` remain supported, but they intentionally stay outside
the compiled cache because they capture runtime arrays.  For compiled execution,
put those arrays directly into `PipelineState` / `OperationState`.

## Reusable randomness and attack-state helpers

`FunAttack_v2` now imports reusable orchestration helpers from the PUF library instead of defining them locally:

- `Pufs.randomness.DeterministicKeyStream`: reproducible JAX PRNG stream for experiments and attacks.
- `Pufs.randomness.DistributionSpec`: immutable distribution metadata and sampling helper for setup-time stochastic work.
- `Pufs.attack_state.AttackContext`: attack runtime context carrying `c_sample`, deterministic keys, and local logs.
- `Pufs.attack_state.AttackRunConfig`: structured attack parameters that can be converted to legacy keyword arguments.
- `Pufs.attack_state.AttackTiming`: serialisable timing bundle for result dictionaries.

These helpers are also re-exported from `Pufs.FunctionalPuf` for convenience.

## Examples

The `examples/` directory contains small scripts for common scientific workflows:

- `01_construct_crp_dataset.py`: deterministic CRP dataset generation through `OperationPipeline`.
